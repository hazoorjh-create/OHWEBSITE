/** Hero id -> name, and hero portrait URLs. Ported from oh_web.py. */
...
export function heroName(hid: number): string {
  return HEROES[hid] ?? `Hero ${hid}`;
}

// Same mapping as bot_rv: hero short-name fixes for the steam CDN portrait path.
const HERO_IMG_FIX: Record<string, string> = {
...
export function heroImg(hid: number): string {
  let short = heroName(hid)
    .toLowerCase()
    .replace(/ /g, "_")
    .replace(/-/g, "")
    .replace(/'/g, "");
  short = HERO_IMG_FIX[short] ?? short;
